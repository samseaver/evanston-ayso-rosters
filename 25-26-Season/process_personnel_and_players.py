...
player_prev_ratings_file = open("../24-25-Season/2024_Player_Ratings.tsv")
player_prev_ratings = csv.DictReader(player_prev_ratings_file,delimiter='\t')
player_prev_ratings_data = {}
for line in player_prev_ratings:
	...
	if line['Rating']!='' and line['Rating']!= "NA - Did not play" and line['Rating']!="NA":
		...
		player_prev_ratings_data[(player_ln, player_fn)] = c_rating

player_ratings_file = open("2025_Player_Ratings.tsv")
...
for line in player_ratings:
	...
	elif((player_ln,player_fn) in player_prev_ratings_data):
		c_rating = player_prev_ratings_data[(player_ln, player_fn)]
		player_ratings_data[(player_ln, player_fn)] = c_rating
...
for line in personnel_data:
	...
	coach_data[coach_name]={"Team Personnel Name": coach_name, \
							...
							"Players": line["associatedPlayers"].split(',')}
...
if("10" in DIVISION or "12" in DIVISION):
	...
	for line in extra_data:
		...
		extra_players[(player_ln,player_fn)]=None
		if('Team' in line):
			extra_players[(player_ln,player_fn)]=line["Team"]
...
for line in player_data:
	...
	c_experience = line["Years of Experience:(18939060)"]
	...
	if(line['Division Price Payment Status'] != "Paid"):
		print("Payment Warning: ",player_fn,player_ln,line['Division Price Payment Status'],line['Primary Contact Email'])
	...
	if (player_ln, player_fn) in player_ratings_data:
		...
		if("Has played competitive soccer" in line["Player's Experience Level(18939062)"]):
			c_player_rating = 4
		if("Has played recreational soccer" in line["Player's Experience Level(18939062)"]):
			c_player_rating = 3
		if("No or limited soccer experience" in line["Player's Experience Level(18939062)"]):
			c_player_rating = 2
...
# get additional pairings if needed
add_pair_file = DIVISION + "/" + DIVISION + "_Add_AssociatedPlayers.txt"
add_pair_data = dict()

if not os.path.isfile(add_pair_file):
	# print(f"WARNING: Add AssociatedPlayers file does not exist: {add_pair_file}")
	pass
else:
	with open(add_pair_file) as f:
		add_pair_lines = [line.strip().lower() for line in f if line.strip()]
		for line in add_pair_lines:
			tmp_lst=line.split('\t')
			add_pair_data[(tmp_lst[0],tmp_lst[1])]=(tmp_lst[3],tmp_lst[2])
...
for line in coach_pairs:
	...
	elif (coach_first,coach_last) in add_pair_data:
		player_name = [add_pair_data[(coach_first,coach_last)]]
	else:
		if coach not in coach_data:
			print("Coach Missing in Personnel Data: "+coach)
			continue

		if(len(coach_data[coach]['Players'])==1 and coach_data[coach]['Players'][0]=="No Answer"):
			print("No associated players for "+coach)
		else:
			for player in coach_data[coach]['Players']:
				stripped_player = tuple(reversed(re.sub(r'\s*\(.*?\)', '', player).lower().split(' ')))
				player_name.append(stripped_player)
...
# For 5U and for 6U we sort players solely by their birth date
if DIVISION == "5U" or DIVISION == "6U":
	sorted_players = sorted(
		players.keys(),
		key=lambda x: (
			0 if players[x]["rating"] is None else players[x]["rating"],
			players[x]["dob"].split("/")[2],
			players[x]["dob"].split("/")[0],
			players[x]["dob"].split("/")[1]
		),
		reverse=True
	)
else:
...
if(len(extra_players)>0):
	...
	for c_player in extra_players:
		...
		while not player_added and i < len(c_teams):
			if len(teams[c_teams[i]]["players"])+len(siblings) <= c_max_players: #Used to be 6 as fixed for 10UB
...
for team in teams:
	...
	for player in teams[team]["players"]:
		...
		in_extra=False
		extra_team=""
		if((player_ln,player_fn) in extra_players):
			in_extra=True
			extra_team=extra_players[(player_ln,player_fn)]
		rfh.write("\t".join([team_name,player,str(players[(player_ln, player_fn)]["rating"]),str(in_extra),str(extra_team)])+"\n")

	team_rating = team_rating/float(len(teams[team]["players"]))
	tfh.write(f"{team_name}\t{team_rating:.2f}\n")		
	for coach in teams[team]["coaches"]:
		if(coach not in coach_data):
			print("Skipping coach: ",coach)
			continue
